    ...
    public class MonitoringMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, IRabbitMQService rabbitMQService, IAccessTokenService accessTokenService, IUserService userService)
        {
            ...
            if (attribute != null)
            {
                ...
                switch (partHttpContext)
                {
                    ...
                    case PartHttpContextEnum.RequestBody:
                        try
                        {
                            ...
                            {
                                dynamic json = requestContent as dynamic;
                                entityUid = json.uid;
                            }
                            else entityUid = null;
                        }
                ...
                AuthorizePoliciesEnum policy = (AuthorizePoliciesEnum)Enum.Parse(typeof(AuthorizePoliciesEnum), attributeAuth.Policy);

                bool isAdmin = false;
                if (attributeAuth.Roles is not null)
                ...
                string login = "аноним";
                if (Guid.Empty != userUid)
                {
                    var user = await userService.GetUserByUidAsync(userUid);
                    login = user.Data.Login;
                }
                string action = $"Пользователь {login} в {now} попытался получить доступ к " +
                ...
                if ((entity == EntitiesEnum.Secret || entity == EntitiesEnum.Storage) && entityUid is not null && request.Method != "GET")
                {
                    context.Items.Add(nameof(message.Entity), message.Entity);
                    context.Items.Add(nameof(message.EntityUid), message.EntityUid);
                    context.Items.Add(nameof(request.Method), request.Method);
                    context.Items.Add(nameof(message.UserUid), message.UserUid);
                    context.Items.Add(nameof(message.StatusCode), message.StatusCode);
                }
            }
        }
    }
